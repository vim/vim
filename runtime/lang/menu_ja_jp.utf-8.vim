...
menutrans Toggle\ Pattern\ &Highlight<Tab>:set\ hls!  パターン強調切替(&H)<Tab>:set\ hls!
menutrans Toggle\ &Ignoring\ Case<Tab>:set\ ic! 大小文字区別切替(&I)<Tab>:set\ ic!
menutrans Toggle\ &Showing\ Matched\ Pairs<Tab>:set\ sm!	マッチ表示切替(&S)<Tab>:set\ sm!
...
menutrans Always	常時
" }}}
menutrans Toggle\ Insert\ &Mode<Tab>:set\ im!	挿入(初心者)モード切替(&M)<Tab>:set\ im!
menutrans Toggle\ Vi\ C&ompatibility<Tab>:set\ cp!	Vi互換モード切替(&O)<Tab>:set\ cp!
...
" Boolean options
menutrans Toggle\ Line\ &Numbering<Tab>:set\ nu!	行番号表示切替(&N)<Tab>:set\ nu!
menutrans Toggle\ Relati&ve\ Line\ Numbering<Tab>:set\ rnu!	相対行番号表示切替(&V)<Tab>:set\ rnu!
menutrans Toggle\ &List\ Mode<Tab>:set\ list!	リストモード切替(&L)<Tab>:set\ list!
menutrans Toggle\ Line\ &Wrapping<Tab>:set\ wrap!	行折返し切替(&W)<Tab>:set\ wrap!
menutrans Toggle\ W&rapping\ at\ Word<Tab>:set\ lbr!	単語折返し切替(&R)<Tab>:set\ lbr!
menutrans Toggle\ Tab\ &Expanding<Tab>:set\ et!	タブ展開切替(&E)<Tab>:set\ et!
menutrans Toggle\ &Auto\ Indenting<Tab>:set\ ai!	自動字下げ切替(&A)<Tab>:set\ ai!
menutrans Toggle\ &C-Style\ Indenting<Tab>:set\ cin!	C言語字下げ切替(&C)<Tab>:set\ cin!
