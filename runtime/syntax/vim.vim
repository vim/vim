...
syn keyword vimCommand contained wu[ndo] wv[iminfo] x[it] xa[ll] xr[estore] y[ank] nextgroup=vimBang
...
" Commands {{{2
syn cluster vimCmdList	contains=@vim9CmdList,@vimExUserCmdList,vimAbb,vimAt,vimAugroup,vimAutocmd,vimBehave,vimBreakadd,vimBreakdel,vimBreaklist,vimBuffer,vimCall,vimCatch,vimCd,vimCommand,vimCommandModifier,vimConst,vimCopy,vimDebug,vimDebuggreedy,vimDef,vimDefFold,vimDefer,vimDelcommand,vimDelete,vimDelfunction,vimDoCommand,vimDoautocmd,vimDotdocFold,@vimEcho,vimElse,vimElseif,vimEnddef,vimEndfunction,vimEndif,vimEquals,vimEval,vimExFilter,vimExecute,vimExtCmd,vimFiletype,vimFor,vimFunction,vimFunctionFold,vimGlobal,vimGrep,vimGrepadd,vimHelp,vimHelpgrep,vimHighlight,vimHistory,vimIf,vimImport,vimLanguage,vimLet,vimLoadkeymap,vimLockvar,vimLua,vimMake,vimMap,vimMark,vimMatch,vimMenu,vimMenutranslate,vimMzScheme,vimNormal,vimPerl,vimPound,vimProfdel,vimProfile,vimPrompt,vimPython,vimPython3,vimPythonX,vimRedir,vimReturn,vimRuby,vimSet,vimShiftLeft,vimShiftRight,vimSleep,vimSort,vimSubst,vimSynColor,vimSynLink,vimSyntax,vimSyntime,vimTcl,vimTerminal,vimThrow,vimUniq,vimUnlet,vimUnlockvar,vimUnmap,vimUserCmd,vimUsrCmd,vimVimgrep,vimVimgrepadd,vimWhile,vimWincmd,vimWrite
...
if &buftype != 'nofile'
  syn match	vimCommand	contained	"\<a\%[ppend]\>"	skipwhite skipnl skipempty nextgroup=vimDotdocBang,@vimDotdocText,vimDotdocComment,vimDotdocBar
  syn match	vimCommand_	contained	"a\%[ppend]\>"	skipwhite skipnl skipempty nextgroup=vimDotdocBang,@vimDotdocText,vimDotdocComment,vimDotdocBar
  syn match	vimCommand	contained	"\<c\%[hange]\>"	skipwhite skipnl skipempty nextgroup=vimDotdocBang,@vimDotdocText,vimDotdocComment,vimDotdocBar
  syn match	vimCommand_	contained	"c\%[hange]\>"	skipwhite skipnl skipempty nextgroup=vimDotdocBang,@vimDotdocText,vimDotdocComment,vimDotdocBar
  syn match	vimCommand	contained	"\<i\%[nsert]\>"	skipwhite skipnl skipempty nextgroup=vimDotdocBang,@vimDotdocText,vimDotdocComment,vimDotdocBar
  syn match	vimCommand_	contained	"i\%[nsert]\>"	skipwhite skipnl skipempty nextgroup=vimDotdocBang,@vimDotdocText,vimDotdocComment,vimDotdocBar

  syn match	vimDotdocBang	contained	"\a\@1<=!"	skipwhite skipnl skipempty nextgroup=@vimDotdocText,vimDotdocComment,vimDotdocBar
  syn match	vimDotdocText	contained	"|\@1<=.*"	          skipnl skipempty nextgroup=@vimDotdocText
  syn region	vimDotdocText	contained
        \ start="^."
        \ matchgroup=vimCommand
        \ end="^\.$"
        \ extend
  " NOTE: 'autoindent' end marker not supported
  syn match	vimDotdocEnd	contained	"^\.$"
  syn cluster vimDotdocText contains=vimDotdocText,vimDotdocEnd

  syn match	vimDotdocComment	contained
        \ /".*/
        \ skipnl skipempty nextgroup=@vimDotdocText
  syn match	vimDotdocBar	contained
        \ "\s*\zs|"
        \ nextgroup=@vimDotdocText

  if exists("g:vimsyn_folding") && g:vimsyn_folding =~# 'd'
    " excludes "extend" option, handled by wrapper
    syn region	vimDotdocText	contained
          \ start="^."
          \ matchgroup=vimCommand
          \ end="^\.$"
    syn region	vimDotdocFold
          \ start="\l\@1<!a\%[ppend]\>"
          \ start="\l\@1<!c\%[hange]\>"
          \ start="\l\@1<!i\%[nsert]\>"
          \ end="^\.$"
...
syn keyword	vimBreaklist	contained	breakl[ist]	skipwhite nextgroup=vimComment,vim9Comment,vimSep
...
" :language {{{3
" =========
syn keyword 	vimLanguage		contained	lan[guage]	skipwhite nextgroup=@vimLanguageName,vimLanguageCategory,vimSep,vimComment,vim9Comment
...
if !exists("skip_vim_syntax_inits")
 ...
 hi def link vimDotdocBang	vimBang
 hi def link vimDotdocComment	vimComment
 hi def link vimDotdocEnd	vimCommand
 hi def link vimDotdocText	String
