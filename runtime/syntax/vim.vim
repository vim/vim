...
" Function Names {{{2
syn keyword vimFuncName contained	abs argc assert_equal assert_match atan balloon_show bufexists bufwinid ceil ch_getbufnr ch_read ch_status chdir complete_check count deletebufline digraph_set eval exists_compiled extendnew findfile fnameescape foldtextresult get getcharmod getcmdpos getcursorcharpos getftime getmarklist getreg gettabwinvar getwinposy has_key histnr iconv inputlist invert items job_stop keys line2byte listener_remove maparg matchadd matchfuzzypos min or popup_close popup_findinfo popup_locate popup_show prompt_setinterrupt prop_list prop_type_list rand reduce remote_expr remove rubyeval screenrow searchpairpos setcellwidths setenv setqflist sha256 sign_jump simplify sound_playevent split str2nr strgetchar strridx swapname system taglist term_dumpwrite term_getline term_list term_setrestore test_autochdir test_ignore_error test_null_job test_refcount timer_info toupper undofile visualmode win_gotoid win_splitmove winline wordcount
syn keyword vimFuncName contained	acos argidx assert_equalfile assert_nobeep atan2 balloon_split buflisted bufwinnr ch_canread ch_getjob ch_readblob changenr cindent complete_info cscope_connection did_filetype digraph_setlist eventhandler exp feedkeys flatten fnamemodify foreground getbufinfo getcharpos getcmdscreenpos getcwd getftype getmatches getreginfo gettagstack getwinvar haslocaldir hlID indent inputrestore isabsolutepath job_getchannel join keytrans lispindent localtime mapcheck matchaddpos matchlist mkdir pathshorten popup_create popup_findpreview popup_menu pow prompt_setprompt prop_remove pum_getpos range reg_executing remote_foreground rename screenattr screenstring searchpos setcharpos setfperm setreg shellescape sign_place sin sound_playfile sqrt strcharlen stridx strtrans synID systemlist tan term_getaltscreen term_getscrolled term_scrape term_setsize test_feedinput test_mswin_event test_null_list test_setmouse timer_pause tr undotree wildmenumode win_id2tabwin winbufnr winnr writefile
syn keyword vimFuncName contained	add arglistid assert_exception assert_notequal autocmd_add blob2list bufload byte2line ch_close ch_info ch_readraw char2nr clearmatches confirm cursor diff_filler echoraw executable expand filereadable flattennew foldclosed fullcommand getbufline getcharsearch getcmdtype getenv getimstatus getmousepos getregtype gettext glob hasmapto hlexists index inputsave isdirectory job_info js_decode len list2blob log maplist matcharg matchstr mode perleval popup_dialog popup_getoptions popup_move prevnonblank prop_add prop_type_add pumvisible readblob reg_recording remote_peek repeat screenchar search server2client setcharsearch setline settabvar shiftwidth sign_placelist sinh sound_stop srand strcharpart string strwidth synIDattr tabpagebuflist tanh term_getansicolors term_getsize term_sendkeys term_start test_garbagecollect_now test_null_blob test_null_partial test_settime timer_start trim uniq win_execute win_id2win wincol winrestcmd xor
syn keyword vimFuncName contained	and argv assert_fails assert_notmatch autocmd_delete browse bufloaded byteidx ch_close_in ch_log ch_sendexpr charclass col copy debugbreak diff_hlID empty execute expandcmd filewritable float2nr foldclosedend funcref getbufvar getcharstr getcmdwintype getfontname getjumplist getpid getscriptinfo getwininfo glob2regpat histadd hlget indexof inputsecret isinf job_setoptions js_encode libcall list2str log10 mapnew matchdelete matchstrpos mzeval popup_atcursor popup_filter_menu popup_getpos popup_notification printf prop_add_list prop_type_change py3eval readdir reltime remote_read resolve screenchars searchcount serverlist setcmdline setloclist settabwinvar sign_define sign_undefine slice soundfold state strchars strlen submatch synIDtrans tabpagenr tempname term_getattr term_getstatus term_setansicolors term_wait test_garbagecollect_soon test_null_channel test_null_string test_srand_seed timer_stop trunc values win_findbuf win_move_separator windowsversion winrestview
syn keyword vimFuncName contained	append asin assert_false assert_report autocmd_get browsedir bufname byteidxcomp ch_evalexpr ch_logfile ch_sendraw charcol complete cos deepcopy digraph_get environ exepath expr10 filter floor foldlevel function getchangelist getcmdcompltype getcompletion getfperm getline getpos gettabinfo getwinpos globpath histdel hlset input insert islocked job_start json_decode libcallnr listener_add luaeval mapset matchend max nextnonblank popup_beval popup_filter_yesno popup_hide popup_setoptions prompt_getprompt prop_clear prop_type_delete pyeval readdirex reltimefloat remote_send reverse screencol searchdecl setbufline setcmdpos setmatches settagstack sign_getdefined sign_unplace sort spellbadword str2float strdisplaywidth strpart substitute synconcealed tabpagewinnr term_dumpdiff term_getcursor term_gettitle term_setapi terminalprops test_getvalue test_null_dict test_option_not_set test_unknown timer_stopall type virtcol win_getid win_move_statusline winheight winsaveview
syn keyword vimFuncName contained	appendbufline assert_beeps assert_inrange assert_true balloon_gettext bufadd bufnr call ch_evalraw ch_open ch_setoptions charidx complete_add cosh delete digraph_getlist escape exists extend finddir fmod foldtext garbagecollect getchar getcmdline getcurpos getfsize getloclist getqflist gettabvar getwinposx has histget hostname inputdialog interrupt isnan job_status json_encode line listener_flush map match matchfuzzy menu_info nr2char popup_clear popup_findecho popup_list popup_settext prompt_setcallback prop_find prop_type_get pyxeval readfile reltimestr remote_startserver round screenpos searchpair setbufvar setcursorcharpos setpos setwinvar sign_getplaced sign_unplacelist sound_clear spellsuggest str2list strftime strptime swapinfo synstack tagfiles term_dumpload term_getjob term_gettty term_setkill test_alloc_fail test_gui_event test_null_function test_override test_void tolower typename virtcol2col win_gettype win_screenpos winlayout winwidth
