...
if exists("g:vimsyn_folding") && g:vimsyn_folding =~# '[afhilmpPrt]'
...
" =======================
if &buftype != 'nofile'
 syn match  vimInsert	"^[: \t]*\%(\d\+\(,\d\+\)\=\)\=a\%[ppend]\>" skipwhite skipnl nextgroup=vimInsertBang,vimInsertText
 syn match  vimInsert	"^[: \t]*\%(\d\+\(,\d\+\)\=\)\=c\%[hange]\>" skipwhite skipnl nextgroup=vimInsertBang,vimInsertText
 syn match  vimInsert	"^[: \t]*\%(\d\+\(,\d\+\)\=\)\=i\%[nsert]\>" skipwhite skipnl nextgroup=vimInsertBang,vimInsertText
 VimFoldi syn region vimInsertText	contained start="^" matchgroup=vimCommand end="^\.$"
 syn match  vimInsertBang	contained "\a\@1<=!" skipwhite skipnl nextgroup=vimInsertText
endif
...
if !exists("skip_vim_syntax_inits")
 ...
 hi def link vimInsert	vimCommand
 hi def link vimInsertBang	vimBang
 hi def link vimInsertText	vimString
