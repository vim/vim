...
" Set up a syntax error command for shell {{{1
" =======================================
if exists("g:sh_no_error_rules") && type(g:sh_no_error_rules) == type([])
    fun! s:DefineSyntaxErrorRule(rule) abort
        " Read shFooError from either (syn, match, shFooError, "regexp") etc.
        " or (hi, def, link, shFooError).
        let parts = matchlist(a:rule, '\(\(\S\+\)\s\+\(\S\+\)\s*\)\{2}')
        " Do not define rules for names listed in "g:sh_no_error_rules".
        if len(parts) < 4 || index(g:sh_no_error_rules, parts[(parts[2] ==# "link") ? 3 : 2]) < 0
            exec a:rule
        endif
    endfun
elseif exists("g:sh_no_error")
    fun! s:DefineSyntaxErrorRule(_)
    endfun
else
    fun! s:DefineSyntaxErrorRule(rule) abort
        exec a:rule
...
com! -nargs=+ ShDefSynErrRule call s:DefineSyntaxErrorRule(<q-args>)
...
if !exists("skip_sh_syntax_inits")
    ...
    ShDefSynErrRule hi def link shCaseError		Error
    ShDefSynErrRule hi def link shCondError		Error
    ShDefSynErrRule hi def link shCurlyError		Error
    ShDefSynErrRule hi def link shDerefOpError		Error
    ShDefSynErrRule hi def link shDerefWordError		Error
    ShDefSynErrRule hi def link shDoError		Error
    ShDefSynErrRule hi def link shEsacError		Error
    ShDefSynErrRule hi def link shIfError		Error
    ShDefSynErrRule hi def link shInError		Error
    ShDefSynErrRule hi def link shParenError		Error
    ShDefSynErrRule hi def link shTestError		Error
    if exists("b:is_bash")
        ShDefSynErrRule hi def link shFunctionNameAssignError	Error
        ShDefSynErrRule hi def link shFunctionNameCommentError	Error
    endif
    if exists("b:is_kornshell") || exists("b:is_posix")
        ShDefSynErrRule hi def link shDTestError		Error
    endif
...
delfun s:DefineSyntaxErrorRule
