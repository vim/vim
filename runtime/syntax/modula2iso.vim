...
"
" Variable g:m2iso_allow_lowline controls support for lowline in identifiers:
" if defined and set to a non-zero value, they are recognised, otherwise not
"
" Variable g:m2iso_disallow_octals controls the rendering of octal literals:
" if defined and set to a non-zero value, they are rendered as errors.
"
" Variable g:m2iso_disallow_synonyms controls the rendering of @, & and ~:
" if defined and set to a non-zero value, they are rendered as errors.
"
...
syn keyword m2Resword ELSIF EXCEPT EXIT EXPORT FINALLY FOR FORWARD FROM IF
syn keyword m2Resword IMPLEMENTATION IMPORT IN LOOP MOD NOT OF OR PACKEDSET
syn keyword m2Resword POINTER QUALIFIED RECORD REPEAT REM RETRY RETURN SET
syn keyword m2Resword THEN TO TYPE UNTIL VAR WHILE WITH


" -----------------------------------------------------------------------------
" Builtin Constant Identifiers
" -----------------------------------------------------------------------------
syn keyword m2ConstIdent FALSE NIL TRUE INTERRUPTIBLE UNINTERRUPTIBLE
...
syn keyword m2TypeIdent COMPLEX LONGCOMPLEX PROTECTION
...
syn keyword m2FuncIdent ABS CHR CMPLX FLOAT HIGH IM INT LENGTH LFLOAT MAX MIN
syn keyword m2FuncIdent ODD ORD RE SIZE TRUNC VAL
...
" -----------------------------------------------------------------------------
syn keyword m2UnsafeIdent ADDRESS BYTE LOC WORD
syn keyword m2UnsafeIdent ADR CAST TSIZE SYSTEM
syn keyword m2UnsafeIdent MAKEADR ADDADR SUBADR DIFADR ROTATE SHIFT


" -----------------------------------------------------------------------------
" Non-Portable Language Extensions
" -----------------------------------------------------------------------------
syn keyword m2NonPortableIdent LONGCARD LONGBITSET
...
syn match m2DialectTag "(\*!m2iso\(+[a-z0-9]\+\)\?\*)"

" -----------------------------------------------------------------------------
" Block Comments
" -----------------------------------------------------------------------------
syn region m2Comment start="(\*\(!m2iso\(+[a-z0-9]\+\)\?\*)\)\@!" end="\*)"
...
" lowline identifiers are rendered as errors if g:m2iso_allow_lowline is unset
if exists("g:m2iso_allow_lowline")
  if g:m2iso_allow_lowline != 0
...
" octal literals are rendered as errors if g:m2iso_disallow_octals is set
if exists("g:m2iso_disallow_octals")
  if g:m2iso_disallow_octals != 0
...
" synonyms & and ~ are rendered as errors if g:m2iso_disallow_synonyms is set
if exists("g:m2iso_disallow_synonyms")
  if g:m2iso_disallow_synonyms != 0
