...
test = f"result: {value:{fragments}{of}{format}{specs}}"
test = f"{number:#0x}"
test = f"{number:+#0x}"
test = f"{number:<+#0x}"
test = f"{number: <+#0x}"
...
test = f"Lambda returns {(lambda x: x + 1)}"
test = f"Zero padded lambda returns {(lambda x: x + 1):09}"
test = f"Space padded lambda returns {(lambda x: x + 1):{width}}"
test = f"List copy is {s[:]}"
test = f"List slice is {s[1:]}"
test = f"List slice is {s[:9]}"
test = f"Formatted list elements are {s[:2:]}"
test = f"Formatted list copy is {s[:]:99}"
test = f"Formatted list slice is {s[1:]:<99}"
test = f"Formatted list slice is {s[:9]:>99}"
test = f"Formatted list elements are {s[:2:]:^99}"
