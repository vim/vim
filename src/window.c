...
check_lnums(int do_curwin)
{
	...
	if ((do_curwin || wp != curwin) && wp->w_buffer == curbuf)
	{
	    ...
	    wp->w_save_cursor.w_cursor_init = wp->w_cursor;
	    wp->w_save_cursor.w_topline_init = wp->w_topline;
	}
}
...
reset_lnums()
{
	...
	if (wp->w_buffer == curbuf)
	{
	    if (EQUAL_POS(wp->w_save_cursor.w_cursor_init, wp->w_cursor)
		    && !EQUAL_POS(wp->w_save_cursor.w_cursor_save, wp->w_cursor))
		wp->w_cursor = wp->w_save_cursor.w_cursor_save;
	    if (wp->w_save_cursor.w_topline_init == wp->w_topline
		    && wp->w_save_cursor.w_topline_save != wp->w_topline)
