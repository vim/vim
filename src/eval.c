...
/*
 * Returns TRUE if "*arg" points to the type arguments passed to a generic
 * function call.  For a generic function call, "*arg" points to "<", followed
 * by one or more types, the closing ">" and the opening "(".  If TRUE, on
 * return, "*arg" points to "(".  If FALSE, then "arg" is not modified.
 */
    static int
    ...
    char_u	**name_start)
{
	...
	else if (evaluate)
	{
		...
		*name_start = s;
		...
		if (ret == OK && vim9script &&
			**arg == '<' && rettv->v_type == VAR_FUNC)
		{
		    // skip generic function arguments
		    *arg += 1;
		    char_u *p = generic_func_find_close_angle_bracket(*arg);
		    if (p != NULL)
			*arg = p + 1;	// skip ">"
		    else
			ret = FAIL;
