...
wayland_init_connection(const char *display)
{
    ...
    wayland_display_fd = wayland_ct->display.fd;
...
/*
 * Dispatch events from the queue of the global Wayland connection display.
 * Returns OK on success and FAIL on failure
 */
    static int
wayland_poll_dispatch(void)
{
    return wl_display_dispatch_pending(wayland_ct->display.proxy)
	== -1 ? FAIL : OK;
}

/*
 * Should be called before polling the global Wayland connection display fd.
 * Returns OK on success and FAIL on failure.
 */
    int
wayland_poll_pre(void)
{
    if (wayland_ct == NULL)
	return FAIL;
    while (wl_display_prepare_read(wayland_ct->display.proxy) == -1)
	// Event queue not empty, dispatch
	if (wayland_poll_dispatch() == FAIL)
	    return FAIL;
    return vwl_connection_flush(wayland_ct);
}
...
/*
 * Called after polling the global Wayland connection display fd.
 *
 * "state" is either
 *  0: no events happened
 *  1: success
 * -1: fail (connection lost)
 */
    void
wayland_poll_post(int state)
{
    if (wayland_ct == NULL)
	return;
    switch(state)
...
vwl_data_device_set_selection(
    vwl_data_device_T *self, vwl_data_source_T *source, uint32_t serial,
