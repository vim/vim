    ...
    int		infile)
{
	...
	iemsg((char_u *)_("E293: block was not locked"));
    flags &= ~BH_LOCKED;
    if (dirty)
    {
	flags |= BH_DIRTY;
...
mf_write(memfile_T *mfp, bhdr_T *hp)
{
	...
	if (mf_write_block(mfp, hp2 == NULL ? hp : hp2, offset, size) == FAIL)
	{
		...
		emsg((char_u *)_("E297: Write error in swap file"));
    ...
    int		flags)		/* flags for open() */
{
    ...
    {
	mfp->mf_fd = -1;
	emsg((char_u *)_("E300: Swap file already exists (symlink attack?)"));
    }
    else
#endif
    {
