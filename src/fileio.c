...
#define CRYPT_BUFFER_SIZE (WRITEBUFSIZE + 36 + crypto_secretstream_xchacha20poly1305_ABYTES)
#endif
...
# ifdef CRYPT_NOT_INPLACE
		    ...
		    {
# endif
			crypt_decode_inplace(curbuf->b_cryptstate, ptr, size, eof);
# ifdef CRYPT_NOT_INPLACE
		    }
		    else
		    {
			...
			decrypted_size = crypt_decode_alloc(
				    curbuf->b_cryptstate, ptr, size, &newptr, eof);
