...
popup_options_one(dict_T *dict, char_u *key)
{
	...
	n = screen_screencol() + 1 + n - TSB_LCOL(NULL);
...
popup_adjust_position(win_T *wp)
{
    ...
    if (center_hor)
    {
	wp->w_wincol = (Columns - wp->w_width - extra_width - TSB_LCOL(NULL)) / 2;
	if (wp->w_wincol < 0)
	    wp->w_wincol = 0;
    }
...
update_popups(void (*win_update)(win_T *wp))
{
	...
	left_extra = wp->w_popup_padding[3] + wp->w_popup_border[3] + TSB_LCOL(NULL)
							 - wp->w_popup_leftoff;
	...
	title_wincol = wp->w_wincol + 1 + TSB_LCOL(NULL);
	...
	wincol = wp->w_wincol - wp->w_popup_leftoff + TSB_LCOL(NULL);
	...
	if (top_padding > 0 || wp->w_popup_padding[2] > 0)
	{
	    ...
	    padendcol = wp->w_wincol + total_width - wp->w_popup_border[1] + TSB_LCOL(NULL)
							 - wp->w_has_scrollbar;
	    if (padcol < 0)
	    {
		padendcol += padcol;
				 ...
				 i < total_height - wp->w_popup_border[2]; ++i)
	{
	    ...
	    {
		int line = i - top_off;
		int scroll_col = wp->w_wincol + total_width - 1 + TSB_LCOL(NULL)
						       - wp->w_popup_border[1];
