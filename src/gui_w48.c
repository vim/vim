 ...
 */
static void outputDeadKey_rePost(MSG originalMsg) {
    ...
    PostMessage(originalMsg.hwnd, originalMsg.message, originalMsg.wParam, originalMsg.lParam);
}
...
process_message(void)
{
	...
	for (i = 0; special_keys[i].key_sym != 0; i++)
	{
		 ...
		 * the TAB key, etc...).
		 */
		if (dead_key && (special_keys[i].vim_code0 == 'K' || vk == VK_TAB || vk == CAR)) {
		    outputDeadKey_rePost(msg);
		    return;
		}
