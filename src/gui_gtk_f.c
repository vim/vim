...
#include "vim.h"
#include <gtk/gtk.h> // without this it compiles, but gives errors at runtime!
#include "gui_gtk_f.h"
...
#if GTK_CHECK_VERSION(3, 0, 0)
static void form_get_preferred_width(GtkWidget *widget,
                                     gint *minimal_width,
                                     gint *natural_width);
static void form_get_preferred_height(GtkWidget *widget,
                                      gint *minimal_height,
                                      gint *natural_height);
#endif
...
static void form_forall(GtkContainer *container,
...
static void form_position_child(GtkForm *form,
                                GtkFormChild *child,
...
void
gui_gtk_form_put(GtkForm *form, GtkWidget *child_widget, gint x, gint y)
{
    GtkFormChild *child;
...
void
gui_gtk_form_move(GtkForm *form, GtkWidget *child_widget, gint x, gint y)
{
    GList *tmp_list;
    GtkFormChild *child;
    ...
    while (tmp_list)
    {
        ...
        if (child->window != NULL)
        {
            g_signal_handlers_disconnect_by_func(
                G_OBJECT(child->widget), FUNC2GENERIC(form_child_map), child);
            g_signal_handlers_disconnect_by_func(
                G_OBJECT(child->widget), FUNC2GENERIC(form_child_unmap), child);
...
form_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
    ...
    if (gtk_widget_get_realized(widget))
    {
        gdk_window_move_resize(gtk_widget_get_window(widget), allocation->x,
                               ...
                               allocation->height);
        gdk_window_move_resize(GTK_FORM(widget)->bin_window, 0, 0,
...
form_expose(GtkWidget *widget, GdkEventExpose *event)
{
    ...
    for (tmp_list = form->children; tmp_list; tmp_list = tmp_list->next)
        gtk_container_propagate_expose(
            ...
            GTK_WIDGET(((GtkFormChild *)tmp_list->data)->widget), event);
...
#endif
        child->window =
            gdk_window_new(form->bin_window, &attributes, attributes_mask);
...
#else
        gtk_style_set_background(widget->style, child->window,
                                 GTK_STATE_NORMAL);
#endif
...
form_position_child(GtkForm *form, GtkFormChild *child, gboolean force_allocate)
{
    ...
    if ((x >= G_MINSHORT) && (x <= G_MAXSHORT) && (y >= G_MINSHORT) &&
        (y <= G_MAXSHORT))
    {
        if (!child->mapped)
        {
        ...
        if (force_allocate)
        {
            ...
            if (!gtk_widget_get_has_window(child->widget))
            {
                if (child->window)
                {
                    gdk_window_move_resize(child->window, x, y,
                                           requisition.width,
                                           requisition.height);
                }
...
void
gui_gtk_form_move_resize(GtkForm *form,
                         GtkWidget *widget,
                         gint x,
                         gint y,
                         gint w,
