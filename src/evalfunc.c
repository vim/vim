...
f_exists_info(typval_T *argvars, typval_T *rettv)
{
	...
	if (argname != NULL)
	{
	    char_u  *end = vim_strchr(argname, ',');

	    name = argname;
	    namelen = end == NULL ? (int)STRLEN(argname) : (int)(end - argname);
	    argname = end == NULL ? NULL : skipwhite(end + 1);
	}
	arg = dict_alloc();
	...
	if (arg == NULL || types == NULL
		...
		|| (namelen > 0
		    && dict_add_string_len(arg, "name", name, namelen) == FAIL)
