...
typedef struct {
    size_t count;
    unsigned char key[crypto_box_SEEDBYTES];  // 32, same as crypto_secretstream_xchacha20poly1305_KEYBYTES
...
crypt_encode_inplace(
    ...
    int         last)
{
    cryptmethods[state->method_nr].encode_inplace_fn(state, buf, len, buf, last);
}
...
crypt_decode_inplace(
    ...
    int		last)
{
    cryptmethods[state->method_nr].decode_inplace_fn(state, buf, len, buf, last);
}
...
# ifdef FEAT_SODIUM
    ...
    sodium_state_T   *sd_state = (sodium_state_T *)sodium_malloc(sizeof(sodium_state_T));
...
# ifdef FEAT_SODIUM
    ...
    unsigned char  tag = last ? crypto_secretstream_xchacha20poly1305_TAG_FINAL  : 0;
	...
	crypto_secretstream_xchacha20poly1305_init_push(&sod_st->state, to, sod_st->key);
    ...
    crypto_secretstream_xchacha20poly1305_push
	 (&sod_st->state, to, NULL, from, len, NULL, 0, tag);
...
# ifdef FEAT_SODIUM
    ...
    if (sod_st->count == 0 && len <= crypto_secretstream_xchacha20poly1305_HEADERBYTES)
    {
    ...
    if (sod_st->count == 0)
    {
	if (crypto_secretstream_xchacha20poly1305_init_pull(&sod_st->state, from, sod_st->key) != 0) {
    ...
    if (crypto_secretstream_xchacha20poly1305_pull(&sod_st->state, buf_out, &buf_len, &tag, from, len, NULL, 0) != 0)
    {
...
# ifdef FEAT_SODIUM
    ...
    *buf_out = alloc_clear(length);
    ...
    sod_st->count++;
    return out_len + (first ? crypto_secretstream_xchacha20poly1305_HEADERBYTES : 0);
# else
    return -1;
# endif
}
...
# ifdef FEAT_SODIUM
    ...
    *buf_out = alloc_clear(len);
    ...
    if (crypto_secretstream_xchacha20poly1305_pull(&sod_st->state, *buf_out, &out_len, &tag, from, len, NULL, 0) != 0)
    {
	emsg(e_libsodium_decription_failed);
	return -1;
    }
