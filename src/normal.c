 ...
 */
int do_mouse(oparg_T *oap, /* operator argument, can be NULL */
        ...
        if (VIsual_active) {
          ...
          if (mouse_row < curwin->w_winrow ||
            ...
            if ((LT_POS(curwin->w_cursor, VIsual) &&
                 (LT_POS(m_pos, curwin->w_cursor) || LT_POS(VIsual, m_pos))) ||
                (LT_POS(VIsual, curwin->w_cursor) &&
                 (LT_POS(m_pos, VIsual) || LT_POS(curwin->w_cursor, m_pos)))) {
              jump_flags = MOUSE_MAY_STOP_VIS;
