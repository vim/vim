...
# endif
    ...
    if (*p_opfunc == NUL)
	emsg((char_u *)_("E774: 'operatorfunc' is empty"));
    else
    {
...
#else
    emsg((char_u *)_("E775: Eval feature not available"));
#endif
}
    ...
    int		find_type)
{
	...
	if (find_type & FIND_STRING)
	    emsg((char_u *)_("E348: No string under cursor"));
	else
	    emsg((char_u *)_(e_noident));
	return 0;
    }
    ptr += col;
    *string = ptr;
    ...
    case 'E':	if (foldmethodIsManual(curwin))
		{
		    ...
		    emsg((char_u *)_("E352: Cannot erase folds with current 'foldmethod'"));
		break;
...
nv_ident(cmdarg_T *cap)
{
    ...
    {
	emsg((char_u *)_(e_noident));	 /* found white space only */
	return;
    }
    kp_ex = (*kp == ':');
	...
	case 'K':
		...
		if (n == 0)
		{
		    emsg((char_u *)_(e_noident));	 /* found dashes only */
		    vim_free(buf);
		    return;
		}
...
# endif
	...
	else if (cap->cmdchar == 'g')
	{
		...
		emsg((char_u *)_("E664: changelist is empty"));
	    else if (cap->count1 < 0)
		emsg((char_u *)_("E662: At start of changelist"));
	    else
		emsg((char_u *)_("E663: At end of changelist"));
	}
	else
	    clearopbeep(cap->oap);
# ifdef FEAT_FOLDING
