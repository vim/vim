...
vgetc(void)
{
	...
	for (;;)		// this is done twice if there are modifiers.
	{
		...
		// Handle K_TEAROFF here, the caller doesn't need to know that
		// a menu was torn off.
		if (c == K_TEAROFF)
		{
		    char_u name[200];
		...
		// know about it.  Return K_IGNORE so that we loop once
		// (needed if 'lazyredraw' is set).
		...
		for (i = 1; i < n; ++i)
		{
		    ...
		    {
			// Must be any of sequences:
			//
			// K_SPECIAL - KS_SPECIAL - KE_FILLER
			//     which represents a K_SPECIAL (0x80).
			//
			// CSI - KS_EXTRA - KE_CSI
			//     which represents a CSI (0x9B).
			//
			// K_SPECIAL - KS_EXTRA - KE_CSI
			//     which represents a CSI (0x9B) too.
			c = vgetorpeek(TRUE);
