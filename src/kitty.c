 ...
 *	    Base64 encoding is shared with eval's base64_encode()/decode(),
 *	    see base64.c/base64.h -- no separate table or algorithm here.
 */
...
kitty_transmit(image_rgb_T *img, int id)
{
	...
	if (first)
	{
	    hdr_len = vim_snprintf(buf, sizeof(buf),
		    ...
		    id, fmt, img->width, img->height, more ? 1 : 0);
	    first = FALSE;
	}
	else
 ...
 * If "del_data" is true, then then its data will be freed by the terminal (see
...
kitty_delete(int id, bool del_data)
{
    ...
    vim_snprintf((char *)IObuff, IOSIZE,
	    "\033_Ga=d,d=%c,i=%d,p=%d,q=2\033\\", d_key, id, id);
