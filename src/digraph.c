...
listdigraphs(int use_headers)
{
    ...
    static void
digraph_get_list_appendpair(digr_T *dp, list_T *l)
{
    char_u	buf[30];
    char_u	*p;
    list_T	*l2;
    ...
    *p = NUL;
    ...
    void
digraph_get_list_common(int list_all, typval_T *rettv)
{
    int		i;
    digr_T	*dp;
	...
	for (i = 0; dp->char1 != NUL && !got_int; ++i)
	{
	    ...
	    if (tmp.result != 0 && tmp.result != tmp.char2
					  && (has_mbyte || tmp.result <= 255))
		digraph_get_list_appendpair(&tmp, rettv->vval.v_list);
#else
	    if (getexactdigraph(dp->char1, dp->char2, FALSE) == dp->result
		    && (has_mbyte || dp->result <= 255))
		digraph_get_list_appendpair(dp, rettv->vval.v_list);
#endif
	    ++dp;
	}
    }
...
/*
 * "digraph_get_list()" function
 */
    void
f_digraph_get_list(typval_T *argvars, typval_T *rettv)
{
# ifdef FEAT_DIGRAPHS
    int     flag_list_all;
    ...
    digraph_get_list_common(flag_list_all, rettv);
# else
    emsg(_(e_no_digraphs_version));
# endif
}
...
/*
 * "digraph_set_list()" function
 */
    void
f_digraph_set_list(typval_T * argvars, typval_T *rettv)
{
# ifdef FEAT_DIGRAPHS
    list_T	*pl, *l;
    listitem_T	*pli;
    ...
    {
	emsg(_(e_digraph_set_list_argument_must_be_list_of_lists_with_two_items));
	return;
    }
