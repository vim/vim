...
_IFNDEF_PN_RE = re.compile(r'^\s*#\s*ifndef\s+PROTO_NEW\b')        # drop whole group
_IF_NOTDEF_PN_RE = re.compile(r'^\s*#\s*!defined\s*\(\s*PROTO_NEW\s*\)')
...
def rewrite_conditionals_first_branch(text: str) -> str:
    ...
    while i < n:
        ...
        kw = m.group(1)
        if kw in ("if", "ifdef", "ifndef"):
            if _IF0_RE.match(line) or _IFNDEF_PN_RE.match(line) or _IF_NOTDEF_PN_RE.match(line):
