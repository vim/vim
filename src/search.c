...
// convert tolower simd
#ifdef PLATFORM_X86
static inline __m128i simd_tolower(__m128i r)
{
    __m128i offset = _mm_add_epi8(r, _mm_set1_epi8(63)); //  A = SCHAR_MIN
    __m128i cmp = _mm_cmpgt_epi8(_mm_set1_epi8((char)(-102)), offset);
    __m128i mask = _mm_and_si128(cmp, _mm_set1_epi8(32));
    return _mm_add_epi8(r, mask);
}
#elif defined(PLATFORM_ARM_NEON)
static inline uint8x16_t simd_tolower(uint8x16_t r UNUSED)
{
    uint8x16_t t = vsubq_u8(r, vdupq_n_u8(63));
    t = vcltq_u8(t, vdupq_n_u8(26));
    t = vandq_u8(t, vdupq_n_u8(32));
    return vaddq_u8(r, t);
}
#endif
...
fuzzy_match_simd(const char_u *pattern, const char_u *text)
{
	...
	__m128i text_chunk = _mm_loadu_si128((__m128i *)text_chunk_data);
        text_chunk = simd_tolower(text_chunk);
