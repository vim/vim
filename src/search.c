...
// Just for temporary Debugging. Removed when done.
#define SW_DEBUG 0
...
static void sw_debug_log(const char *format, ...)
{
    ...
    if (!initialized)
    {
	log_file = fopen("sw_debug.log", "w");
	if (log_file)
	{
	    setvbuf(log_file, NULL, _IONBF, 0);
	    fprintf(log_file, "==== SW MATCHING DEBUG LOG ====\n");
...
#define SIMD_FUZZY_MISMATCH_PENALTY    1     // Note: positive value, used in subtraction
#define SIMD_FUZZY_GAP_OPEN_PENALTY    3     // Note: positive value, used in subtraction
...
// Configuration flags
#define SIMD_FUZZY_REQUIRE_ALL_CHARS   1     // Set to 1 to require all needle chars to match (strict)
    ...
    matrix_state_t  start_state)
{
	...
	sw_debug_log("  Backtrack step: (%d,%d) state=%d n_char='%c' h_char='%c'\n", i, j, state, n_char, h_char);
    ...
    matrix_state_t  *max_state)
{
    ...
    // 1. Compute match/mismatch scores
    int match_score = (n_char == h_char) ?
			SIMD_FUZZY_MATCH_SCORE : -SIMD_FUZZY_MISMATCH_PENALTY;

    sw_debug_log("  Base match score: %d\n", match_score);
    ...
    // 2. Compute the M matrix (match state)
    int m_prev = max3(sf->score_matrix_M[idx_prev],
	    sf->score_matrix_X[idx_prev], sf->score_matrix_Y[idx_prev]);
    sf->score_matrix_M[idx] = match_score + m_prev;

    sw_debug_log("  M_prev: %d, M_current: %d\n", m_prev, sf->score_matrix_M[idx]);

    // 3. Compute the X matrix (gaps in the X sequence)
    int x_from_m = sf->score_matrix_M[idx_left] -
	    (SIMD_FUZZY_GAP_OPEN_PENALTY + SIMD_FUZZY_GAP_EXTEND_PENALTY);
    int x_from_x = sf->score_matrix_X[idx_left] -
					    SIMD_FUZZY_GAP_EXTEND_PENALTY;
    int x_from_y = sf->score_matrix_Y[idx_left] -
	    (SIMD_FUZZY_GAP_OPEN_PENALTY + SIMD_FUZZY_GAP_EXTEND_PENALTY);
    ...
    // 4. Compute the Y matrix (gaps in the Y sequence)
    int y_from_m = sf->score_matrix_M[idx_up] -
		(SIMD_FUZZY_GAP_OPEN_PENALTY + SIMD_FUZZY_GAP_EXTEND_PENALTY);
    int y_from_x = sf->score_matrix_X[idx_up] -
		(SIMD_FUZZY_GAP_OPEN_PENALTY + SIMD_FUZZY_GAP_EXTEND_PENALTY);
...
simd_affine_gap_calculate_sse2(
    simd_fuzzy_T *sf,
    int i, int j,
    int idx, int idx_prev, int idx_up, int idx_left,
    int *max_score, int *max_i, int *max_j, matrix_state_t *max_state)
{
    ...
    __m128i match_mask = _mm_unpacklo_epi8(cmp_result, _mm_setzero_si128());
    ...
    __m128i max_prev = _mm_max_epi16(
	    _mm_max_epi16(m_prev, x_prev),
	    y_prev);
    ...
    __m128i x_current = _mm_max_epi16(
	    _mm_max_epi16(x_candidate1, x_candidate2),
	    x_candidate3);
    ...
    __m128i y_current = _mm_max_epi16(
	    _mm_max_epi16(y_candidate1, y_candidate2),
	    y_candidate3);
    ...
    /* Replace alignas with compatible macro */
    VIM_ALIGNAS(16) int16_t m_scores[8];
    VIM_ALIGNAS(16) int16_t x_scores[8];
    VIM_ALIGNAS(16) int16_t y_scores[8];
	...
	if (m_scores[k] > *max_score)
	{
	    ...
	    *max_j = j + k;
	    *max_state = STATE_M;
	    fuzzy_debug_log("  SIMD New max score: M[%d][%d] = %d\n", i, j+k, *max_score);
	}
	if (x_scores[k] > *max_score)
	{
	    ...
	    *max_j = j + k;
	    *max_state = STATE_X;
	    fuzzy_debug_log("  SIMD New max score: X[%d][%d] = %d\n", i, j+k, *max_score);
	}
	if (y_scores[k] > *max_score)
	{
	    ...
	    *max_j = j + k;
	    *max_state = STATE_Y;
	    fuzzy_debug_log("  SIMD New max score: Y[%d][%d] = %d\n", i, j+k, *max_score);
	}
    }
}
    ...
    matrix_state_t  *max_state)
{
    ...
    int16x8_t mismatch_penalties = vandq_s16(
	    vmvnq_s16(vreinterpretq_s16_u16(match_mask)),
    ...
    int16x8_t x_current = vmaxq_s16(
	vmaxq_s16(x_candidate1, x_candidate2),
	x_candidate3
    );
    ...
    int16x8_t y_current = vmaxq_s16(
	vmaxq_s16(y_candidate1, y_candidate2),
	y_candidate3
    );
	...
	if (m_scores[k] > *max_score)
	{
	    ...
	    *max_j = j + k;
	    *max_state = STATE_M;
	    sw_debug_log("  NEON New max score: M[%d][%d] = %d\n", i, j+k, *max_score);
	}
	if (x_scores[k] > *max_score)
	{
	    *max_score = x_scores[k];
	    ...
	    *max_j = j + k;
	    *max_state = STATE_X;
	    sw_debug_log("  NEON New max score: X[%d][%d] = %d\n", i, j+k, *max_score);
	}
	if (y_scores[k] > *max_score)
	{
	    *max_score = y_scores[k];
	    ...
	    *max_j = j + k;
	    *max_state = STATE_Y;
	    sw_debug_log("  NEON New max score: Y[%d][%d] = %d\n", i, j+k, *max_score);
	}
    }
}
#endif
...
/*
 * Handle diagonal band – using SIMD or scalar
 */
    static void
process_diagonal_strip(
    simd_fuzzy_T    *sf,
    int		    diag,
    int		    *max_score,
    int		    *max_i,
    ...
    matrix_state_t  *max_state)
{
	...
	{
	    fuzzy_debug_log("  Using SSE2 SIMD for i=%d, j=%d\n", i, j);
	    simd_affine_gap_calculate_sse2(sf, i, j, idx, idx_prev, idx_up,
				idx_left, max_score, max_i, max_j, max_state);
	    continue;
	}
#elif defined(PLATFORM_ARM_NEON) && (defined(__ARM_NEON) || defined(__ARM_NEON__))
	if (haystack_len - j + 1 >= 4)
	{
	    ...
	    simd_affine_gap_calculate_neon(sf, i, j, idx, idx_prev, idx_up,
				idx_left, max_score, max_i, max_j, max_state);
	    continue;
	}
#endif
	...
	affine_gap_calculate(sf, i, j, idx, idx_prev, idx_up, idx_left,
...
simd_fuzzy_smith_waterman(simd_fuzzy_T *sf)
{
    ...
    if (sf->score_matrix_M == NULL || sf->score_matrix_X == NULL ||
						sf->score_matrix_Y == NULL)
    {
	sw_debug_log("Memory allocation failed\n");
	...
	sf->needle != NULL && sf->haystack != NULL &&
	STRNCMP(sf->needle, sf->haystack, needle_len) == 0)
    {
    ...
    int		    maxMatches)     // maximum number of matches to return
{
	...
	*outScore = SIMD_FUZZY_SCORE_MIN;
    ...
    sf.case_sensitive = ignorecase(pat_arg);
    sw_debug_log("Case sensitive: %d\n", sf.case_sensitive);
...
simd_fuzzy_match_str(char_u *str, char_u *pat)
{
    int score = 0;
    unsigned int matches[SIMD_FUZZY_MAX_MATCHES];
    int match_count = simd_fuzzy_match(str, pat, TRUE, &score, matches,
						    SIMD_FUZZY_MAX_MATCHES);
