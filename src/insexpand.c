...
compl_get_info(char_u *line, int startcol, colnr_T curs_col, int *line_invalid)
{
    ...
    {
	if (get_normal_compl_info(line, startcol, curs_col) == FAIL)
	    return FAIL;
    }
    else if (ctrl_x_mode_line_or_eval())
    {
	if (get_wholeline_compl_info(line, curs_col) == FAIL)
	    return FAIL;
    }
    else if (ctrl_x_mode == CTRL_X_FILES)
    {
	if (get_filename_compl_info(line, startcol, curs_col) == FAIL)
	    return FAIL;
    }
    else if (ctrl_x_mode == CTRL_X_CMDLINE)
    {
	if (get_cmdline_compl_info(line, curs_col) == FAIL)
	    return FAIL;
    }
...
#define	MIN_SPACE 75
	...
	// While getting completion info, if 'line' is changed, then
	// get it again.
	if (line_invalid)
	    line = ml_get(curwin->w_cursor.lnum);
