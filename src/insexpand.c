...
/*
 * Checks whether the pre-insert effect cursor position.
...
{
    if ((get_cot_flags() & (COT_PREINSERT | COT_FUZZY)) != COT_PREINSERT
				|| compl_ins_end_col < curwin->w_cursor.col)
	return FALSE;
...
ins_compl_new_leader(void)
{
    ...
    if (compl_match_array == NULL)
	compl_enter_selects = FALSE;
    else if ((get_cot_flags() & (COT_PREINSERT | COT_FUZZY)) == COT_PREINSERT
						&& compl_leader.length > 0)
...
ins_compl_delete(void)
{
    ...
    if (has_preinsert)
    {
	col = compl_col + ins_compl_leader_len();
...
{
    int	    compl_len = get_compl_len();
    int	    preinsert = (get_cot_flags() & (COT_PREINSERT | COT_FUZZY))
							    == COT_PREINSERT;
    ...
    int	    in_compl_func)	// called from complete_check()
{
    ...
    int	    compl_fuzzy_match = (cur_cot_flags & COT_FUZZY) != 0;
    int	    compl_preinsert = (cur_cot_flags & COT_PREINSERT) != 0 && !compl_fuzzy_match;
