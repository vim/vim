...
ins_ctrl_x(void)
{
	...
	// CTRL-X in CTRL-X CTRL-V mode behaves differently to make CTRL-X CTRL-V
	// look like CTRL-N
...
ins_compl_prep(int c)
{
		...
		|| !vim_is_ctrl_x_key(c))
	{
	    ...
	    // Do not move this assignment before the if condition
	...
	else
	{
	    // Do not move this assignment before the if condition
	    ctrl_x_mode = CTRL_X_CMDLINE;
	    // Other CTRL-X keys first stop completion,
