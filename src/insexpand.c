...
ins_compl_col_range_attr(int col)
{
    ...
    int		scol = compl_col;
    int		fuzzy_filter = (get_cot_flags() & COT_FUZZY) != 0;
    garray_T	*match_pos = NULL;
    int		lead_attr = syn_name2attr((char_u *)"PmenuMatchLead");
    int		ins_attr = syn_name2attr((char_u *)"PmenuMatchIns");
    int		attr = -1;
    int		i = 0;

    if (fuzzy_filter && compl_match_array != NULL
	    && leader !=NULL && leader_len > 0)
    {
	match_pos = fuzzy_match_str_with_pos(compl_shown_match->cp_str.string,
								    leader);
	if (match_pos == NULL)
	    return -1;
	attr = ins_attr;

	for (i = 0 ; i < match_pos->ga_len; ++i)
	{
	    if (col == compl_col + ((int *)match_pos->ga_data)[i])
	    {
		attr = lead_attr;
		break;
	    }
    ...
    if (col >= compl_col && col < scol)
	attr = lead_attr;
    else if (col >= scol && col < compl_ins_end_col)
	attr = ins_attr;
