...
static void recent_func_log_func(const gchar *log_domain,
...
# if GTK_CHECK_VERSION(3, 10, 0)
    ...
    if (pixel_size > 0 || pixel_size == -1)
    {
        GdkPixbuf *const pixbuf = gdk_pixbuf_new_from_file_at_scale(
            (const char *)name, pixel_size, pixel_size, TRUE, NULL);
        if (pixbuf != NULL)
        {
...
#  define ADD_ICON(Name, Data) \
      add_stock_icon(factory, Name, Data, (int)sizeof(Data))
    ...
    for (names = stock_vim_icons; names->icon_name != NULL; names++)
    {
        ...
        if (pixbuf != NULL)
        {
            ...
            if (size > 16)
            {
                ...
                pixbuf =
                    gdk_pixbuf_scale_simple(src, 16, 16, GDK_INTERP_BILINEAR);
                if (pixbuf == NULL)
                    pixbuf = src;
                else
                    g_object_unref(src);
...
# endif
    ...
    if (menu->actext != NULL && menu->actext[0] != NUL)
    {
        ...
        gtk_box_pack_end(GTK_BOX(box), gtk_label_new((const char *)text), FALSE,
                         FALSE, 0);
...
#  else
            menu->id = gtk_toolbar_insert_item(
                toolbar, (const char *)text, (const char *)tooltip, NULL,
                create_menu_icon(menu, gtk_toolbar_get_icon_size(toolbar)),
                G_CALLBACK(&menu_item_activate), menu, idx);
#  endif

            if (gtk_socket_id != 0)
                g_signal_connect(G_OBJECT(menu->id), "focus-in-event",
                                 G_CALLBACK(toolbar_button_focus_in_event),
                                 NULL);
...
# endif
            ...
            gtk_menu_shell_insert(GTK_MENU_SHELL(parent->submenu_id), menu->id,
                                  idx);
...
# else
    // Only set the tooltip when it's valid utf-8.
        gtk_tooltips_set_tip(GTK_TOOLBAR(gui.toolbar)->tooltips, menu->id,
...
gui_mch_set_scrollbar_thumb(scrollbar_T *sb, long val, long size, long max)
{
    ...
    g_signal_handler_unblock(G_OBJECT(adjustment), (gulong)sb->handler_id);
}
...
gui_mch_create_scrollbar(scrollbar_T *sb, int orient)
{
    ...
    sb->handler_id = g_signal_connect(G_OBJECT(adjustment), "value-changed",
...
browse_ok_cb(GtkWidget *widget UNUSED, gpointer cbdata)
{
    ...
    vw->browse_fname = (char_u *)g_strdup(
        gtk_file_selection_get_filename(GTK_FILE_SELECTION(vw->filedlg)));
 ...
 */
char_u *
gui_mch_browse(int saving, // select file to write
...
# else // !USE_FILE_CHOOSER
    ...
    if (gui.filedlg == NULL)
    {
        ...
        gtk_signal_connect(GTK_OBJECT(fs->ok_button), "clicked",
                           GTK_SIGNAL_FUNC(browse_ok_cb), &gui);
        gtk_signal_connect(GTK_OBJECT(fs->cancel_button), "clicked",
                           GTK_SIGNAL_FUNC(browse_cancel_cb), &gui);
        // gtk_signal_connect() doesn't work for destroy, it causes a hang
        gtk_signal_connect_object(GTK_OBJECT(gui.filedlg), "destroy",
                                  GTK_SIGNAL_FUNC(browse_destroy_cb),
 ...
 */
char_u *
gui_mch_browsedir(char_u *title, // title for the window
                  char_u *initdir) // initial directory, NULL for current dir
{
...
#  if GTK_CHECK_VERSION(3, 10, 0)
        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_OK"), GTK_RESPONSE_ACCEPT,
#  else
...
#  endif
    ...
    if (gtk_dialog_run(GTK_DIALOG(dirdlg)) == GTK_RESPONSE_ACCEPT)
        dirname =
            (char_u *)gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dirdlg));
...
create_message_dialog(int type, char_u *title, char_u *message)
{
    ...
    dialog = gtk_message_dialog_new(
        GTK_WINDOW(gui.mainwin), GTK_DIALOG_DESTROY_WITH_PARENT, message_type,
        GTK_BUTTONS_NONE, "%s", (const char *)message);
...
# else
        ...
        gtk_box_pack_end(GTK_BOX(GTK_DIALOG(dialog)->vbox), vbutton_box, TRUE,
                         FALSE, 0);
...
# endif
    ...
    for (idx = 1; idx <= n_buttons; ++idx)
    {
        ...
        {
# if GTK_CHECK_VERSION(3, 10, 0)
            if (button_equal(label, ok[0]))
                label = _("OK");
            else if (button_equal(label, ync[0]))
                label = _("Yes");
            else if (button_equal(label, ync[1]))
                label = _("No");
            else if (button_equal(label, ync[2]))
                label = _("Cancel");
            else if (button_equal(label, "Ok"))
                label = _("OK");
            else if (button_equal(label, "Yes"))
                label = _("Yes");
            else if (button_equal(label, "No"))
                label = _("No");
            else if (button_equal(label, "Cancel"))
                label = _("Cancel");
# else
            if (button_equal(label, ok[0]))
                label = GTK_STOCK_OK;
...
dialog_key_press_event_cb(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
    ...
    if (di->ignore_enter &&
        (event->keyval == GDK_Return || event->keyval == ' '))
        return TRUE;
...
# else
        alignment =
            gtk_alignment_new((float)0.5, (float)0.5, (float)1.0, (float)1.0);
# endif
...
# else
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), alignment, TRUE,
                           FALSE, 0);
# endif
...
#  if !GTK_CHECK_VERSION(3, 10, 0)
                                         ...
                                         "vim-has-im-menu") == NULL)
    {
        ...
        g_object_set_data(G_OBJECT(menu->submenu_id), "vim-has-im-menu",
...
# else
    gtk_menu_popup(GTK_MENU(menu->submenu_id), NULL, NULL,
                   (GtkMenuPositionFunc)NULL, NULL, 3U, gui.event_time);
# endif
}
...
# if GTK_CHECK_VERSION(3, 22, 2)
    ...
    else
    {
        ...
        gtk_menu_popup_at_rect(GTK_MENU(menu->submenu_id), win, &rect,
                               GDK_GRAVITY_SOUTH_EAST, GDK_GRAVITY_NORTH_WEST,
                               (GdkEvent *)&trigger);
    }
# else
    gtk_menu_popup(GTK_MENU(menu->submenu_id), NULL, NULL,
                   &popup_menu_position_func, NULL, 0U,
...
static SharedFindReplace find_widgets = { NULL, NULL, NULL, NULL, NULL,
                                          NULL, NULL, NULL, NULL, NULL };
static SharedFindReplace repl_widgets = { NULL, NULL, NULL, NULL, NULL,
                                          NULL, NULL, NULL, NULL, NULL };

static int
find_key_press_event(GtkWidget *widget UNUSED,
                     GdkEventKey *event,
...
static GtkWidget *
#if GTK_CHECK_VERSION(3, 10, 0)
create_image_button(const char *stock_id UNUSED, const char *label)
#else
...
#endif
{
    ...
    if (stock_id != NULL)
        gtk_box_pack_start(
            GTK_BOX(box),
            gtk_image_new_from_stock(stock_id, GTK_ICON_SIZE_BUTTON), FALSE,
            FALSE, 0);
#endif
    gtk_box_pack_start(GTK_BOX(box), gtk_label_new((const char *)text), FALSE,
                       FALSE, 0);
...
#else
    alignment =
        gtk_alignment_new((float)0.5, (float)0.5, (float)0.0, (float)0.0);
#endif
...
#define CONV(message) convert_localized_message(&conv_buffer, (message))
...
#else
    gtk_table_attach(GTK_TABLE(table), tmp, 0, 1, 0, 1, GTK_FILL, GTK_EXPAND, 2,
                     2);
#endif
    ...
    g_signal_connect_after(G_OBJECT(frdp->what), "key-press-event",
                           G_CALLBACK(find_key_press_event), (gpointer)frdp);
#if GTK_CHECK_VERSION(3, 4, 0)
...
#else
        gtk_table_attach(GTK_TABLE(table), tmp, 0, 1, 1, 2, GTK_FILL,
                         GTK_EXPAND, 2, 2);
#endif
...
#else
        gtk_table_attach(GTK_TABLE(table), frdp->wword, 0, 1023, 2, 3, GTK_FILL,
                         GTK_EXPAND, 2, 2);
#endif
    else
#if GTK_CHECK_VERSION(3, 4, 0)
...
#else
        gtk_table_attach(GTK_TABLE(table), frdp->wword, 0, 1023, 1, 2, GTK_FILL,
                         GTK_EXPAND, 2, 2);
#endif
...
#else
        gtk_table_attach(GTK_TABLE(table), frdp->mcase, 0, 1023, 3, 4, GTK_FILL,
                         GTK_EXPAND, 2, 2);
#endif
    else
#if GTK_CHECK_VERSION(3, 4, 0)
...
#else
        gtk_table_attach(GTK_TABLE(table), frdp->mcase, 0, 1023, 2, 3, GTK_FILL,
                         GTK_EXPAND, 2, 2);
#endif
...
#else
        gtk_table_attach(GTK_TABLE(table), tmp, 1023, 1024, 2, 4, GTK_FILL,
                         GTK_FILL, 2, 2);
#endif
    else
#if GTK_CHECK_VERSION(3, 4, 0)
...
#else
        gtk_table_attach(GTK_TABLE(table), tmp, 1023, 1024, 1, 3, GTK_FILL,
                         GTK_FILL, 2, 2);
#endif
#if GTK_CHECK_VERSION(3, 2, 0)
    ...
    g_signal_connect_swapped(G_OBJECT(frdp->dialog), "delete-event",
