...
{
    if (tuple->tv_type != NULL && tuple->tv_type->tt_member != NULL
	    && check_typval_arg_type(tuple->tv_type->tt_member, tv,
							NULL, 0) == FAIL)
	return FAIL;
...
/*
 * Return a List of items in the tuple starting at index n1 and ending at index
 * n2, inclusive (tuple[n1 : n2])
 */
    list_T *
tuple_slice_to_list(tuple_T *tuple, long n1, long n2)
{
    list_T	*l;

    l = list_alloc_with_items(n2 - n1 + 1);
    if (l == NULL)
	return NULL;
