...
static type_T *find_generic_type_in_cctx(char_u *type_name, cctx_T *cctx);
 ...
 *   gftn_gap  - growarray to store parsed type argument names and their types
    ...
    cctx_T	*cctx)
{
	...
	// parse the type
	type_arg = parse_type(&p, types_gap, NULL, cctx, TRUE);
	if (type_arg == NULL)
	    return NULL;

	// create space for the name and the new type
	if (ga_grow(gftn_gap, 1) == FAIL)
	    return NULL;
	gftn_item = (gf_type_name_T *)gftn_gap->ga_data + gftn_gap->ga_len;
	gftn_gap->ga_len++;
 ...
 *
 * Arguments:
 *   name    - the generic type name to check (single character)
 *   gt_gap  - growarray containing existing generic_T entries for this context
 ...
 */
    static int
generic_name_exists(char_u name, garray_T *gt_gap, cctx_T *cctx)
{
    char_u	generic_name[2];
    typval_T	tv;
    ...
    if (eval_variable_import(generic_name, &tv) == OK)
    {
	semsg(_(e_redefining_script_item_str), generic_name);
	clear_tv(&tv);
	return TRUE;
    }

    for (int i = 0; i < gt_gap->ga_len; i++)
    {
	if (name == ((generic_T *)gt_gap->ga_data)[i].gt_name)
	{
	    semsg(_(e_duplicate_generic_type_str), generic_name);
	    return TRUE;
	}
    }

    if (cctx != NULL && find_generic_type_in_cctx(generic_name, cctx) != NULL)
    {
	semsg(_(e_duplicate_generic_type_str), generic_name);
	return TRUE;
    }
 ...
 *   gtl_gap   - growarray to store the created type_T objects for each
 *		 parameter
 *   gt_gap    - growarray to store the mapping of parameter names to types
    ...
    cctx_T	*cctx)
{
	...
	if (generic_name_exists(*p, gt_gap, cctx))
	    return NULL;
...
/*
 * Initialize a new generic function "fp" using the list of generic types in
 * "gtl_gap" and the generic arguments in "gt_gap".
 *
 * This function:
 ...
 *
 * After this call, the caller should not use or free "gtl_gap" or "gt_gap".
 */
    void
generic_func_init(ufunc_T *fp, garray_T *gtl_gap, garray_T *gt_gap)
{
    fp->uf_flags |= FC_GENERIC;
    fp->uf_generic_argcount = gtl_gap->ga_len;
    fp->uf_generic_type_list = (type_T *)gtl_gap->ga_data;
    ga_init(gtl_gap);		// remove the reference to the types
    fp->uf_generic_args = (generic_T *)gt_gap->ga_data;
    ga_init(gt_gap);		// remove the reference to the args
 ...
 * arguments specified in "gftn_gap".
 *
 * The lookup key is constructed by concatenating the type argument names from
 * "gftn_gap", separated by ", ", and stored in the provided growarray
 ...
 * arguments specified in "gftn_gap". If such an instance does not exist, it is
...
find_generic_func(ufunc_T *ufunc, char_u *name, char_u **argp)
{
    ...
    p = parse_generic_func_type_args(name, *argp - name, *argp,
						&types_ga, &gftn_table,
						NULL);
    if (p != NULL)
    {
