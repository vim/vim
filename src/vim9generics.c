    ...
    cctx_T		*cctx)
{
	...
	type_arg = parse_type(&p,
		&SCRIPT_ITEM(current_sctx.sc_sid)->sn_type_list, NULL, cctx,
		TRUE);
	if (type_arg == NULL)
	    return NULL;
	...
	char_u	cc = *p;
	*p = NUL;
	generic_arg->gt_name = alloc(STRLEN(gt_name) + 1);
	if (generic_arg->gt_name == NULL)
	    return NULL;
	STRCPY(generic_arg->gt_name, gt_name);
	*p = cc;
    ...
    cctx_T		*cctx)
{
	...
	{
	    semsg(_(e_duplicate_generic_type_str), gt_name);
	    return TRUE;
	}
    }
    ...
    cctx_T		*cctx)
{
	...
	{
	    if (ASCII_ISLOWER(*p))
		semsg(_(e_generic_type_name_must_start_with_uppercase_letter_str), p);
	    else
		semsg(_(e_missing_type_after_str), p - 1);
	    return NULL;
	}
	...
	*name_end = cc;
	...
	if (ga_grow(&gfatab->gfat_types, 1) == FAIL)
	    return NULL;
	type_T *gt =
	    &((type_T *)gfatab->gfat_types.ga_data)[gfatab->gfat_types.ga_len];
	gfatab->gfat_types.ga_len++;
...
generic_func_init(ufunc_T *fp, gfargs_tab_T *gfatab)
{
    ...
    fp->uf_generic_argcount = gfatab->gfat_types.ga_len;
    fp->uf_generic_type_list = (type_T *)gfatab->gfat_types.ga_data;
    ga_init(&gfatab->gfat_types);	// remove the reference to the types
...
{
    ga_init2(&gfatab->gfat_types, sizeof(type_T), 10);
...
{
    clear_type_list(&gfatab->gfat_types);
    for (int i = 0; i < gfatab->gfat_args.ga_len; i++)
    {
	generic_T *generic = &((generic_T *)gfatab->gfat_args.ga_data)[i];
