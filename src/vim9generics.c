    ...
    cctx_T	*cctx)
{
	...
	*p = cc;
    ...
    static int
is_generic_name_exists(char_u name, garray_T *gt_gap, cctx_T *cctx)
{
    ...
    cctx_T	*cctx)
{
	...
	if (is_generic_name_exists(*p, gt_gap, cctx))
	    return NULL;
    ...
    char_u	**arg)
{
    ...
    {
	if (**arg == '<')
	    ufunc = find_generic_func(ufunc, name, arg);
	else
	{
 ...
 */
    ufunc_T *
find_generic_func(ufunc_T *ufunc, char_u *name, char_u **arg)
{
    garray_T	gftn_table;
    garray_T	types_ga;
    char_u	*p;
    ...
    p = parse_generic_func_type_args(name, *arg - name, *arg,
						&types_ga, &gftn_table,
						NULL);
    if (p != NULL)
    {
