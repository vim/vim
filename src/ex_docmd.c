    ...
    int		improved)	    /* TRUE for "improved Ex" mode */
{
	...
	else if (ex_pressedreturn && !ex_no_reprint)	/* must be at EOF */
	{
		...
		emsg((char_u *)_("E501: At end-of-file"));
	}
    }
...
# define cmd_cookie cookie
    ...
    {
	emsg((char_u *)_("E169: Command too recursive"));
#ifdef FEAT_EVAL
...
# endif
					    ...
					    && !func_has_ended(real_cookie))))
	{
		...
		emsg((char_u *)_(e_endtry));
	    else if (cstack.cs_flags[cstack.cs_idx] & CSF_WHILE)
		emsg((char_u *)_(e_endwhile));
	    else if (cstack.cs_flags[cstack.cs_idx] & CSF_FOR)
		emsg((char_u *)_(e_endfor));
	    else
		emsg((char_u *)_(e_endif));
	}
    ...
    int		*addr_type_arg)
{
    ...
    if (len == 0)
    {
	emsg((char_u *)_("E175: No attribute specified"));
	return FAIL;
    }
	...
	*argt |= TRLBAR;
	...
	if (STRNICMP(attr, "nargs", attrlen) == 0)
	{
	    ...
	    {
wrong_nargs:
		emsg((char_u *)_("E176: Invalid number of arguments"));
		return FAIL;
	    }
	}
	else if (STRNICMP(attr, "range", attrlen) == 0)
	{
		...
		*argt |= DFLALL;
		...
		{
two_count:
		    emsg((char_u *)_("E177: Count cannot be specified twice"));
		    return FAIL;
		}
		...
		*argt |= (ZEROR | NOTADR);
		...
		{
invalid_count:
		    emsg((char_u *)_("E178: Invalid default value for count"));
		    return FAIL;
		}
	    }
	}
	...
	else if (STRNICMP(attr, "complete", attrlen) == 0)
	{
	    if (val == NULL)
	    {
		emsg((char_u *)_("E179: argument required for -complete"));
		return FAIL;
	    }
	...
	else if (STRNICMP(attr, "addr", attrlen) == 0)
	{
	    *argt |= RANGE;
	    if (val == NULL)
	    {
		emsg((char_u *)_("E179: argument required for -addr"));
		return FAIL;
	    }
...
ex_command(exarg_T *eap)
{
    ...
    {
	emsg((char_u *)_("E182: Invalid command name"));
	return;
    }
    end = p;
    ...
    {
	emsg((char_u *)_("E183: User defined commands must start with an uppercase letter"));
	return;
    }
    ...
    {
	emsg((char_u *)_("E841: Reserved name, cannot be used for user defined command"));
	return;
    }
    else
...
# endif
    {
	emsg((char_u *)_("E468: Completion argument only allowed for custom completion"));
	return FAIL;
    }
...
# if defined(FEAT_EVAL) && defined(FEAT_CMDL_COMPL)
    if ((*complp == EXPAND_USER_DEFINED || *complp == EXPAND_USER_LIST)
							       && arg == NULL)
    {
	emsg((char_u *)_("E467: Custom completion requires a function argument"));
	return FAIL;
    }
...
# endif
	if (first_tabpage->tp_next == NULL)
	    emsg((char_u *)_("E784: Cannot close last tab page"));
	else
	{
...
ex_cd(exarg_T *eap)
{
	...
	if (vim_strchr(p_cpo, CPO_CHDIR) != NULL && curbufIsChanged()
							     && !eap->forceit)
	{
	    emsg((char_u *)_("E747: Cannot change directory, buffer is modified (add ! to override)"));
	    return;
	}
	...
	if (STRCMP(new_dir, "-") == 0)
	{
	    if (prev_dir == NULL)
	    {
		emsg((char_u *)_("E186: No previous directory"));
		return;
	    }
	    new_dir = prev_dir;
	}
...
do_exmap(exarg_T *eap, int isabbrev)
{
    ...
    {
	case 1: emsg((char_u *)_(e_invarg));
		break;
	case 2: emsg((char_u *)(isabbrev ? _(e_noabbr) : _(e_nomap)));
		break;
    }
}
...
ex_winsize(exarg_T *eap)
{
	...
	emsg((char_u *)_("E465: :winsize requires two number arguments"));
}
...
# endif
	    emsg((char_u *)_("E188: Obtaining window position not implemented for this platform"));
    }
    else
    {
	x = getdigits(&arg);
	...
	if (*p == NUL || *arg != NUL)
	{
	    emsg((char_u *)_("E466: :winpos requires two number arguments"));
	    return;
	}
# ifdef FEAT_GUI
	if (gui.in_use)
...
ex_redir(exarg_T *eap)
{
    ...
    if (redir_execute)
    {
	emsg((char_u *)_("E930: Cannot use :redir inside execute()"));
	return;
    }
#endif
...
ex_mark(exarg_T *eap)
{
	...
	if (setmark(*eap->arg) == FAIL)	/* set mark */
	    emsg((char_u *)_("E191: Argument must be a letter or forward/backward quote"));
...
ex_normal(exarg_T *eap)
{
    ...
    {
	emsg((char_u *)_("E192: Recursive use of :normal too deep"));
	return;
    }
    ...
    exarg_T	*eap)
{
	...
	if (read_viminfo(eap->arg, VIF_WANT_INFO | VIF_WANT_MARKS
	    ...
	    emsg((char_u *)_("E195: Cannot open viminfo file for reading"));
    }
    else
...
#else
    emsg((char_u *)_("E196: No digraphs in this version"));
#endif
}
