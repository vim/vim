...
static char_u	*get_loop_line(int c, void *cookie, int indent, int dont_concat);
 ...
 */
    static char_u *
get_loop_line(int c, void *cookie, int indent, int dont_concat)
{
	...
	if (cp->getline == NULL)
	    line = getcmdline(c, 0L, indent, dont_concat);
	else
	    line = cp->getline(c, cp->cookie, indent, dont_concat);
