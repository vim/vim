...
test_vim_snprintf(void)
{
	...
	assert(n == 7);
	assert(bsize == 0 || STRNCMP(buf, "1234567", bsize - 1) == 0);
	...
	assert(n == 8);
	assert(bsize == 0 || STRNCMP(buf, "deadbeef", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');
	...
	assert(n == 12);
	assert(bsize == 0 || STRNCMP(buf, "1.234000e+00", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');

	n = vim_snprintf(buf, bsize, "%f", 0.0/0.0);
	assert(n == 3);
	assert(bsize == 0 || STRNCMP(buf, "nan", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');

	n = vim_snprintf(buf, bsize, "%f", 1.0/0.0);
	assert(n == 3);
	assert(bsize == 0 || STRNCMP(buf, "inf", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');
	...
	assert(n == 6);
	assert(bsize == 0 || STRNCMP(buf, "漢語", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');
	...
	assert(n == 3);
	assert(bsize == 0 || STRNCMP(buf, "漢", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');

	n = vim_snprintf(buf, bsize, fmt_5S, "foo");
	assert(n == 5);
	assert(bsize == 0 || STRNCMP(buf, "  foo", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');

	n = vim_snprintf(buf, bsize, "%%%%%%");
	assert(n == 3);
	assert(bsize == 0 || STRNCMP(buf, "%%%", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');

	n = vim_snprintf(buf, bsize, "%c%c", 1, 2);
	assert(n == 2);
	assert(bsize == 0 || STRNCMP(buf, "\x01\x02", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');
	...
	assert(n == 10);
	assert(bsize == 0 || STRNCMP(buf, "0x87654321", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');

	n = vim_snprintf(buf, bsize, fmt_012p, ptr);
	assert(n == 12);
	assert(bsize == 0 || STRNCMP(buf, "0x0087654321", bsize - 1) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize - 1)] == '\0');
