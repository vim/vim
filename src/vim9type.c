...
get_tuple_type(type_T *member_type, garray_T *type_gap)
{
    ...
    if (member_type == NULL || member_type->tt_type == VAR_ANY)
	return &t_tuple_any;
    else if (member_type->tt_type == VAR_VOID
	    || member_type->tt_type == VAR_UNKNOWN)
	return &t_tuple_empty;
    else if (member_type->tt_type == VAR_BOOL)
	return &t_tuple_bool;
    else if (member_type->tt_type == VAR_NUMBER)
	return &t_tuple_number;
    else if (member_type->tt_type == VAR_STRING)
...
tuple_typval2type(typval_T *tv, int copyID, garray_T *type_gap, int flags)
{
    ...
    // Use the common type of all members.
    member_type = typval2type(TUPLE_ITEM(tuple, 0), copyID, type_gap,
							TVTT_DO_MEMBER);
    for (int i = 1; i < len; i++)
	common_type(typval2type(TUPLE_ITEM(tuple, i), copyID, type_gap,
