	...
	void			*user_data)
{
    ...
    if (vwl_connection_dispatch(wayland_ct) == -1)
	goto fail;
    ...
    while (vwl_connection_dispatch(wayland_ct) != -1)
    {
	if (store->got_focus)
	    break;
...
wl_data_offer_listener_event_offer(
    void *data UNUSED, vwl_data_offer_T *offer UNUSED,
    const char *mime_type
)
{
    // Only accept mime type if we support it
    for (int i = 0; i < ARRAY_LENGTH(supported_mimes); i++)
	...
	wayland_selection_T selection)
{
    ...
    static void
vwl_data_device_listener_event_finished(void *data, vwl_data_device_T *device)
{
...
clip_wl_request_selection(Clipboard_T *cbd)
{
	...
	// offer.
	if (vwl_connection_dispatch(wayland_ct) == FAIL)
	    goto clear;
#endif
    ...
    for (int i = 0; i < ARRAY_LENGTH(supported_mimes)
	    && chosen_mime == NULL; i++)
    {
    ...
    if (vwl_connection_flush(wayland_ct) == OK)
