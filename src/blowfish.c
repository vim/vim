    ...
    int		salt_len)
{
    ...
    if (keylen == 0)
    {
	iemsg((char_u *)_("E831: bf_key_init() called with empty password"));
	return;
    }
    for (i = 0; i < keylen; i++)
    {
...
bf_self_test(void)
{
	...
	err++;
	emsg((char_u *)_("E820: sizeof(uint32_t) != 4"));
    }
	...
	if (memcmp(bk.uc, bf_test_data[i].cryptxt, 8) != 0)
	{
		...
		emsg((char_u *)_("E817: Blowfish big/little endian use wrong"));
	    err++;
	}
    }
...
blowfish_self_test(void)
{
    ...
    {
	emsg((char_u *)_("E818: sha256 test failed"));
	return FAIL;
    }
    if (bf_self_test() == FAIL)
    {
	emsg((char_u *)_("E819: Blowfish test failed"));
	return FAIL;
    }
    return OK;
}
