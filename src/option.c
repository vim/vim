...
/*
 * Return the prefix type for the option name in *argp.  Returns 1 if there is
 * no prefix, 0 for "no" prefix and 2 for "inv" prefix.
 */
    static int
    ...
    size_t	errbuflen)
{
	...
	flags = options[opt_idx].flags;
	varp =
	    get_varp_scope(&(options[opt_idx]), opt_flags);
    }
    else
    {
	flags = P_STRING;
