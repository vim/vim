...
bt_recognize_char_class(char_u *start, char_u *end)
{
    ...
    // 'ignorecase' applies at execution time, so [a-z] may need to match as
    // [a-zA-Z], and the old engine has no NFA_LOWER_IC: the bare [a-z] and
    // [A-Z] ranges are left as collections.  The classes below have no case
    // or hold both.
    switch (config)
    {
	case CLASS_o9:
