...
f_autocmd_get(typval_T *argvars, typval_T *rettv)
{
	...
	FOR_ALL_AUTOCMD_PATTERNS(event, ap)
	{
		...
		if (DICT_ADD_STRING_LEN_KEYLITERAL(event_dict, "event",
			event_name.string, (int)event_name.length) == FAIL
			|| DICT_ADD_STRING_LEN_KEYLITERAL(event_dict, "group",
			    group_name.string, (int)group_name.length) == FAIL
			|| (ap->buflocal_nr != 0
				&& (DICT_ADD_NUMBER_KEYLITERAL(event_dict, "bufnr",
						    ap->buflocal_nr) == FAIL))
			|| DICT_ADD_STRING_KEYLITERAL(event_dict, "pattern",
							      ap->pat) == FAIL
			|| DICT_ADD_STRING_KEYLITERAL(event_dict, "cmd", ac->cmd) == FAIL
			|| DICT_ADD_BOOL_KEYLITERAL(event_dict, "once", ac->once) == FAIL
			|| DICT_ADD_BOOL_KEYLITERAL(event_dict, "nested",
							   ac->nested) == FAIL)
		{
		    vim_free(pat);
		    return;
