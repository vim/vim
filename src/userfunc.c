...
/*
 * Returns a pointer to the opening angle bracket in "name" skipping the
 * initial <SNR> (if present>.
 */
    char_u *
    ...
    funcexe_T	*funcexe)	// various values
{
	...
	if (parse_generic_func_type_args(name, len, name + len, &types_ga, &gftn_table)
								== NULL)
	    goto ret_free;
    }
...
generic_func_add(ufunc_T *fp, char_u *key, garray_T *gftn_gap)
{
    ...
    STRCAT(new_fp->uf_name, "<");
    STRCAT(new_fp->uf_name, key);
    STRCAT(new_fp->uf_name, ">");
...
{
    if (gftn_gap == NULL || gftn_gap->ga_len < fp->uf_generic_count)
    {
