...
#define MAX_FUNC_NESTING 50
	...
	else
	{
		...
		if (vim9_function && *p == '<')
		{
		    ...
		    char_u *t = generic_func_find_close_angle_bracket(p + 1);
		    if (t == NULL)
		    {
			semsg(_(e_missing_closing_angle_bracket_in_generic_function), p);
			goto theend;
		    }
		    p = t + 1;
		}
    ...
    funcexe_T	*funcexe)	// various values
{
	...
	if (len != -1 && name[len] == '<')
	{
	    ...
	    if (parse_generic_func_type_args(name, len, name + len,
					&types_ga, &gftn_table, NULL) == NULL)
		goto ret_free;
	}
    }
	...
	cctx_T	    *cctx)
{
    ...
    garray_T	default_args;
    garray_T	newlines;
    garray_T	generic_type_list;
    garray_T	generic_args;
    int		varargs = FALSE;
    int		flags = 0;
    char_u	*ret_type = NULL;
    ufunc_T	*fp = NULL;
    ...
    ga_init2(&generic_type_list, sizeof(type_T), 10);
    ga_init2(&generic_args, sizeof(generic_T), 10);
    ...
    {
	// generic function
	p = parse_generic_func_type_params(name, p, &generic_type_list,
							&generic_args, cctx);
	if (p == NULL)
	    goto ret_free;
    }
	...
	if (generic_type_list.ga_len > 0)
	{
	    // initialize generic function state
	    flags |= FC_GENERIC;
	    fp->uf_flags |= FC_GENERIC;
	    fp->uf_generic_argcount = generic_type_list.ga_len;
	    fp->uf_generic_type_list = (type_T *)generic_type_list.ga_data;
	    ga_init(&generic_type_list);	// remove references to the data
	    fp->uf_generic_args = (generic_T *)generic_args.ga_data;
	    ga_init(&generic_args);		// remove reference to the data
...
ret_free:
    ga_clear_strings(&argtypes);
    ga_clear(&arg_objm);
    ga_clear(&generic_type_list);
    ga_clear(&generic_args);
    vim_free(fudi.fd_newkey);
    if (name != name_arg)
	vim_free(name);
    vim_free(ret_type);
...
find_func_by_name(char_u *name, compiletype_T *compile_type)
{
	...
	*compile_type = CT_DEBUG;
	...
	{
	    bracket_start = arg;
	    arg = generic_func_find_close_angle_bracket(arg + 1);
	    if (arg != NULL)
		arg++;
	    else
		arg = bracket_start;
	...
	if (bracket_start == NULL)
	    bracket_start = name + STRLEN(name);

	ufunc = eval_generic_func(ufunc, name, &bracket_start);
    }
