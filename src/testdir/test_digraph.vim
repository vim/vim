...
func Test_digraph_set_list_function()
  call digraph_set_list([['aa', 'き'], ['bb', 'く']])
  call assert_equal('き', digraph_get('aa'))
  call assert_equal('く', digraph_get('bb'))

  call assert_fails('call digraph_set_list([[]])', 'E1216:')
  call assert_fails('call digraph_set_list([["aa", "b", "cc"]])', '1216:')
  call assert_fails('call digraph_set_list([["あ", "あ"]])', 'E1214: Digraph must be just two characters: あ')
endfunc

func Test_digraph_get_list_function()
  " Make sure user-defined digraphs are defined
  call digraph_set_list([['aa', 'き'], ['bb', 'く']])

  for pair in digraph_get_list(1)
    call assert_equal(digraph_get(pair[0]), pair[1])
  endfor

  " We don't know how many digraphs are registered before, so check the number
  " of digraphs returned.
  call assert_equal(digraph_get_list()->len(), digraph_get_list(0)->len())
  call assert_notequal((digraph_get_list()->len()), digraph_get_list(1)->len())
endfunc
