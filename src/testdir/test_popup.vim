...
func Test_pum_matchins_higlight()
  ...
  let lines =<< trim END
    ...
    endfunc
    set omnifunc=Omni_test
    hi PmenuMatchIns ctermfg=red
    hi PmenuMatchLead ctermfg=blue
  END
  call writefile(lines, 'Xscript', 'D')
  let buf = RunVimInTerminal('-S Xscript', {})
  ...
  call TermWait(buf)
  call term_sendkeys(buf, ":set cot+=fuzzy\<CR>Shello hero\<CR>ho\<C-X>\<C-N>")
  call VerifyScreenDump(buf, 'Test_pum_matchins_06', {})
  call term_sendkeys(buf, "\<C-E>\<Esc>")

  call TermWait(buf)
  call term_sendkeys(buf, "ggdGS\<C-X>\<C-O>")
