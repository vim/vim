...
" Check getmaps() return a list of the same size.
" Check mode-lhs is unique for every entry
" Check every dict from getmaps() equals dict from maparg()
def TheGetmapsTest()
  new
  def ClearMaps()
  ...
  # Use getmaps to get all the maps
  var maps_getmaps = getmaps()
  assert_equal(len(maps_command), len(maps_getmaps))

  # make sure all the mode-lhs are unique, no duplicates
  var map_set: dict<number>
  for d in maps_getmaps
    map_set[d.mode .. "-" .. d.lhs .. "-" .. d.buffer] = 0
  endfor
  assert_equal(len(maps_getmaps), len(map_set))

  # For everything returned by getmaps, should be the same as from maparg.
