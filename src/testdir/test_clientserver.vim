...
func Test_client_server_x11_and_socket_server()
  if !has('socketserver') || !has('X11')
    throw 'Skipped: both socket server and X11 features must be compiled'
  endif
  ...
  for type in types
    ...
    let actual_cmd = cmd .. ' --servername ' .. name
    let actual_cmd .= ' --clientserver ' .. type
    ...
    call assert_match(name, system(cmd .. ' --clientserver ' .. type .. ' --servername ' .. name .. ' --remote-expr "v:servername"'))

    call system(cmd .. ' --clientserver ' .. type .. ' --servername ' .. name .. " --remote-expr 'execute(\"qa!\")'")
    try
      call WaitForAssert({-> assert_equal("dead", job_status(job))})
    finally
      if job_status(job) != 'dead'
...
func Test_client_server_socket_server_gui()
  if !has('socketserver') || !has('gui_gtk')
    throw 'Skipped: both socket server and GTK GUI features must be compiled'
  endif
