...
def g:ScreenDumpDiscardFFFDChars(
  ...
  for lstr: string in keys(state)
    ...
    const fst_fffd: number = stridx(testdump[lnum], "\xef\xbf\xbd")
    ...
    writefile([printf('D:  PRE: %d', fst_fffd)], $VIM_SYNTAX_TEST_LOG, 'a')
    ...
    if fst_fffd >= 0
      # Look for the previous cell and potentially the cursor cell.
      const prev_pipe: number = strridx(testdump[lnum], '|', fst_fffd)
      const prev_grth: number = strridx(testdump[lnum], '>', fst_fffd)
      const prefix: number = (prev_grth >= 0 && prev_pipe >= 0 &&
		(prev_grth == 0 || testdump[lnum][prev_grth - 1] != '|'))
	  ? max([prev_pipe, prev_grth])
	  : (prev_pipe >= 0)
	      ? prev_pipe
	      : (prev_grth >= 0)
		  ? prev_grth
		  : fst_fffd
      # Non-equal cells of the line may vary among generated files.
